  ...
  return (
    <div className="p-6">
     <div className="bg-gray-900 text-white p-10 rounded-xl shadow-lg">
          ...
          <Link to="/movies" className="bg-red-600 inline-block mt-5 px-4 py-2 rounded-md font-semibold">Explore Movies</Link>
     </div>
     <h2 className="text-2xl font-bold mt-7 mb-5 text-center text-blue-500 text-shadow-lg text-shadow-blue-600 animate-pulse">Trending Movies</h2>
     <div className="grid grid-cols-4 gap-4">
       {movies.map((movie)=>(
      <div key={movie.id} className="shadow-lg rounded-xl p-4">
        <img src={`https://image.tmdb.org/t/p/w300/${movie.poster_path}`}alt={movie.title} />
        <h3 className="font-semibold mt-3 text-xl text-center">{movie.title}</h3>
      </div>
       ))}
     </div>
    </div>
